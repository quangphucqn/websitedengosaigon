...
export function ProductDetailPage() {
  ...
  return <div className="shell page-space product-detail">
    ...
    <div className="product-detail-grid">
      ...
      <section className="product-detail-copy"><p className="side-label">{categoryLabel[product.category]}</p><h1>{product.name}</h1><p className="detail-price">{formatMoney(product.price)}</p><p className="detail-description">{product.description}</p><div className="product-notes"><p><Check size={17} aria-hidden="true" /> Gỗ tự nhiên, hoàn thiện thủ công</p><p><Check size={17} aria-hidden="true" /> Bóng đèn ánh sáng vàng ấm</p></div><button type="button" className="button add-button" onClick={addToCart}>{added ? <><Check size={19} aria-hidden="true" /> Đã thêm vào giỏ</> : <><ShoppingBag size={19} aria-hidden="true" /> Thêm vào giỏ</>}</button><p className="shipping-note">Giao hàng nội thành 2–4 ngày · Đóng gói cẩn thận</p></section>
    </div>
  </div>
}
